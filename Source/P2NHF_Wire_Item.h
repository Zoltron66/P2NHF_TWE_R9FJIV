...
*   egyedi tulajdonsaggal, ami a hossz (vezetek hossza).
...
*	another unique property, which is the length (cable length).
...
// ======================================================================================================
class TWE_API P2NHF_Wire_Item : public P2NHF_Item
{
private:
	unsigned Length;

public:
	P2NHF_Wire_Item() : Length(0) {}
	P2NHF_Wire_Item(const P2NHF_Item* Param);
	virtual ~P2NHF_Wire_Item() {}

	// Accessor methods for Length
	void SetLength(const unsigned& Param);
	float GetData() const override final { return float(Length); }
